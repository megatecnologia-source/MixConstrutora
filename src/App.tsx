  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-bg-main/95 backdrop-blur-md py-3 shadow-sm' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button 
          className={`lg:hidden touch-target flex items-center justify-center transition-colors duration-500 ${scrolled ? 'text-teal-deep' : 'text-white'}`} 
...
const Hero = () => {
  return (
    <section id="home" className="relative h-[85vh] md:h-screen flex items-center overflow-hidden">
      {/* Background Video */}
        ...
        >
          <div className="max-w-full md:max-w-xl">
            ...
            >
              <a 
                href="#contato"
                className="btn-primary bg-white !text-brand-dark hover:!bg-brand-action hover:!text-white flex items-center justify-center gap-2 group touch-target"
              >
                Falar com um Consultor
              ...
              <a 
                href="#portfolio"
                className="px-6 md:px-10 py-3 md:py-5 text-xs md:text-sm font-bold uppercase tracking-widest text-white border-2 border-white/30 rounded-[24px] hover:bg-white/10 transition-all flex items-center justify-center touch-target"
              >
                Ver Obras
              </a>
            </motion.div>
      ...
      {/* Scroll Indicator */}
      <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 z-10 hidden md:block">
        <motion.div 
          animate={{ y: [0, 10, 0] }}
        ...
        >
          {services.map((service, index) => (
            <motion.div 
              key={index}
              variants={fadeIn}
              className="card-modern p-6 md:p-8 lg:p-10 group relative overflow-hidden flex flex-col"
            >
              <div className="relative z-10 flex-grow">
                <div className="w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-brand-medium/10 text-brand-medium rounded-xl lg:rounded-2xl flex items-center justify-center mb-6 lg:mb-8 group-hover:bg-brand-medium group-hover:text-white transition-all duration-500">
                  {service.icon}
                </div>
                <h3 className="text-base md:text-lg lg:text-xl font-bold mb-3 lg:mb-4 uppercase tracking-tight text-teal-deep">{service.title}</h3>
                <p className="text-teal-deep/60 leading-relaxed font-medium text-xs md:text-sm">
                  {service.description}
                </p>
              </div>
              <div className="absolute -right-8 -bottom-8 w-24 md:w-28 lg:w-32 h-24 md:h-28 lg:h-32 bg-brand-medium/5 organic-blob group-hover:scale-150 transition-transform duration-700" />
            </motion.div>
          ))}
        </motion.div>
      </div>
...
const Stats = () => {
  ...
  return (
    <section className="bg-teal-deep py-16 md:py-24 lg:py-32 relative overflow-hidden diagonal-section">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 lg:gap-20 items-center">
          <motion.div
            ...
            variants={fadeIn}
          >
            <span className="text-brand-medium uppercase tracking-widest text-xs md:text-sm font-bold mb-3 md:mb-4 block">Capacidade Técnica</span>
            <h2 className="text-2xl md:text-4xl lg:text-6xl font-bold text-white mb-6 md:mb-8 leading-tight">
              Estrutura, equipe e equipamentos para grandes contratos.
            </h2>
            <p className="text-white/70 text-sm md:text-lg font-medium leading-relaxed mb-8 md:mb-12">
            ...
            <div className="grid grid-cols-2 gap-6 md:gap-8 lg:gap-12">
              {stats.map((stat, index) => (
                <div key={index} className="flex flex-col gap-2 md:gap-3">
                  ...
                  <span className="text-3xl md:text-4xl lg:text-5xl font-display font-bold text-white">{stat.value}</span>
                  <span className="text-[10px] md:text-xs uppercase tracking-widest text-white/40 font-bold">{stat.label}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <div className="aspect-[3/4] md:aspect-[4/5] rounded-[24px] md:rounded-[40px] overflow-hidden shadow-2xl">
              <img 
            ...
            <div className="absolute -bottom-6 md:-bottom-10 -left-4 md:-left-10 bg-brand-action p-6 md:p-8 lg:p-12 rounded-[20px] md:rounded-[32px] hidden md:block max-w-xs shadow-2xl">
              <p className="text-white font-display italic text-sm md:text-xl leading-snug">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-20 gap-4 md:gap-8">
          <motion.div
            ...
            variants={fadeIn}
          >
            <span className="text-brand-medium uppercase tracking-widest text-xs md:text-sm font-bold mb-3 md:mb-4 block">Obras Realizadas</span>
            <h2 className="text-2xl md:text-4xl lg:text-6xl font-bold text-teal-deep leading-tight">
              Projetos que demonstram solidez e execução.
            </h2>
          </motion.div>
          <a href="#" className="text-brand-action font-bold uppercase tracking-widest text-xs md:text-sm border-b-2 border-brand-action pb-2 hover:text-brand-medium hover:border-brand-medium transition-all touch-target">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-12">
          {projects.map((project, index) => (
            ...
            >
              <div className="aspect-[4/3] md:aspect-[3/4] overflow-hidden rounded-[20px] md:rounded-[32px] bg-mineral mb-4 md:mb-8 relative shadow-lg">
                <img 
                  src={project.image} 
                  alt={project.title}
              ...
              <span className="text-brand-medium uppercase tracking-widest text-[10px] md:text-xs font-bold block mb-2 md:mb-3">{project.type}</span>
              <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-teal-deep mb-1 md:mb-2 group-hover:text-brand-medium transition-colors">{project.title}</h3>
              <p className="text-teal-deep/50 font-medium text-sm md:text-base">{project.location}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
const TechnicalPillars = () => {
  ...
  return (
    <section className="py-16 md:py-24 lg:py-32 bg-brand-medium/5 diagonal-section">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 lg:gap-16">
          {pillars.map((pillar, index) => (
            ...
            >
              <div className="w-16 h-16 md:w-18 md:h-18 lg:w-20 lg:h-20 bg-teal-deep text-white rounded-full flex items-center justify-center mb-6 md:mb-8 shadow-xl group hover:scale-110 transition-transform">
                {pillar.icon}
              </div>
              <h3 className="text-base md:text-lg lg:text-xl font-bold text-teal-deep mb-3 md:mb-4 uppercase tracking-tight">{pillar.title}</h3>
              <p className="text-teal-deep/60 text-sm md:text-base font-medium leading-relaxed max-w-xs">
                {pillar.description}
              </p>
            </motion.div>
          ))}
    ...
    <section className="py-16 md:py-24 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <motion.div 
          ...
          viewport={{ once: true }}
          variants={fadeIn}
          className="text-center max-w-2xl md:max-w-3xl mx-auto mb-12 md:mb-24"
        >
          <span className="text-brand-medium uppercase tracking-widest text-xs md:text-sm font-bold mb-3 md:mb-4 block">Diferenciais Técnicos</span>
          <h2 className="text-2xl md:text-4xl lg:text-6xl font-bold text-teal-deep">Por que gestores confiam na Mix Construtora.</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 lg:gap-16">
          {items.map((item, index) => (
            ...
            >
              <div className="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 bg-brand-medium/10 text-brand-medium rounded-xl md:rounded-2xl flex items-center justify-center group-hover:bg-brand-medium group-hover:text-white transition-all duration-500">
                {item.icon}
              </div>
              <div>
                <h3 className="text-base md:text-lg lg:text-xl font-bold uppercase tracking-tight mb-2 md:mb-3 text-teal-deep">{item.title}</h3>
                <p className="text-teal-deep/60 text-sm md:text-base font-medium leading-relaxed">{item.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
...
const Contact = () => {
  ...
  return (
    <section id="contato" className="py-16 md:py-24 lg:py-32 bg-bg-main diagonal-section">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 lg:gap-24">
          <motion.div
            ...
            variants={fadeIn}
          >
            <span className="text-brand-medium uppercase tracking-widest text-xs md:text-sm font-bold mb-3 md:mb-4 block">Contato</span>
            <h2 className="text-2xl md:text-4xl lg:text-6xl font-bold text-teal-deep mb-6 md:mb-10 leading-tight">
              Solicite um orçamento ou agende uma visita técnica.
            </h2>
            <p className="text-teal-deep/60 text-sm md:text-xl font-medium mb-10 md:mb-16">
            ...
            <div className="space-y-6 md:space-y-10">
              <div className="flex items-start gap-4 md:gap-6">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-brand-medium/10 rounded-xl flex items-center justify-center text-brand-medium flex-shrink-0">
              ...
              <div className="flex items-start gap-4 md:gap-6">
                ...
                <div>
                  <span className="block text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40 mb-1 md:mb-2">E-mail</span>
                  <p className="text-lg md:text-2xl font-bold text-teal-deep">contato@mixconstrutora.com.br</p>
                </div>
              </div>
              <div className="flex items-start gap-4 md:gap-6">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-brand-medium/10 rounded-xl flex items-center justify-center text-brand-medium flex-shrink-0">
                ...
                <div>
                  <span className="block text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40 mb-1 md:mb-2">Endereço</span>
                  <p className="text-lg md:text-2xl font-bold text-teal-deep">Rodovia Br 316, 37 Bairro: Centro - Bacabal, MA</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="bg-white p-6 md:p-10 lg:p-12 xl:p-16 rounded-[24px] md:rounded-[32px] lg:rounded-[48px] shadow-2xl border border-teal-deep/5 mt-8 lg:mt-0"
          >
            <form name="contact" data-netlify="true" netlify-honeypot="bot-field" onSubmit={handleSubmit} className="space-y-5 md:space-y-8">
              <input type="hidden" name="form-name" value="contact" />
              <p className="hidden"><input name="bot-field" /></p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                <div className="space-y-2 md:space-y-3">
                  <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">Nome Completo</label>
                  <input type="text" name="nome" value={formData.nome} onChange={handleChange} required className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium touch-target" placeholder="Seu nome" />
                </div>
                <div className="space-y-2 md:space-y-3">
                  <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">Empresa / Órgão</label>
                  <input type="text" name="empresa" value={formData.empresa} onChange={handleChange} className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium touch-target" placeholder="Nome da instituição" />
                </div>
              </div>
              <div className="space-y-2 md:space-y-3">
                <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">Tipo de Serviço</label>
                <select name="servico" value={formData.servico} onChange={handleChange} className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium appearance-none touch-target">
              ...
              <div className="space-y-2 md:space-y-3">
                <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">Descrição do Projeto</label>
                <textarea name="descricao" value={formData.descricao} onChange={handleChange} rows={3} md:rows={4} className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium resize-none touch-target" placeholder="Conte-nos sobre sua necessidade"></textarea>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                <div className="space-y-2 md:space-y-3">
                  <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">Telefone</label>
                  <input type="tel" name="telefone" value={formData.telefone} onChange={handlePhoneChange} required minLength={14} className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium touch-target" placeholder="(00) 00000-0000" />
                </div>
                <div className="space-y-2 md:space-y-3">
                  <label className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-teal-deep/40">E-mail</label>
                  <input type="email" name="email" value={formData.email} onChange={handleChange} required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" className="w-full bg-bg-main rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 text-teal-deep focus:ring-2 focus:ring-brand-medium outline-none transition-all font-medium touch-target" placeholder="seu@email.com" />
                </div>
              </div>
              <div className="pt-4 md:pt-6">
                <button type="submit" disabled={status === 'loading'} className="btn-primary w-full flex items-center justify-center gap-2 md:gap-3 text-sm md:text-lg disabled:opacity-50 disabled:cursor-not-allowed">
...
const Footer = () => {
  return (
    <footer className="bg-teal-deep text-white py-16 md:py-20 lg:py-24 relative">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12 lg:gap-20 mb-16 md:mb-20 lg:mb-24">
          ...
          <div>
            <h4 className="text-[10px] md:text-xs uppercase tracking-widest font-bold mb-6 md:mb-10 text-brand-medium">Institucional</h4>
            <ul className="space-y-3 md:space-y-5 text-sm md:text-base font-medium opacity-60">
              <li><a href="#" className="hover:text-brand-medium transition-colors touch-target inline-block">Quem Somos</a></li>
              <li><a href="#" className="hover:text-brand-medium transition-colors touch-target inline-block">Nossa Estrutura</a></li>
              <li><a href="#" className="hover:text-brand-medium transition-colors touch-target inline-block">Certificações</a></li>
              <li><a href="#portfolio" className="hover:text-brand-medium transition-colors touch-target inline-block">Portfólio</a></li>
            </ul>
          </div>

          <div>
            <h4 className="text-[10px] md:text-xs uppercase tracking-widest font-bold mb-6 md:mb-10 text-brand-medium">Serviços</h4>
            <ul className="space-y-3 md:space-y-5 text-sm md:text-base font-medium opacity-60">
              <li><a href="#servicos" className="hover:text-brand-medium transition-colors touch-target inline-block">Urbanização</a></li>
              <li><a href="#servicos" className="hover:text-brand-medium transition-colors touch-target inline-block">Gestão de Resíduos</a></li>
              <li><a href="#servicos" className="hover:text-brand-medium transition-colors touch-target inline-block">Obras Civis</a></li>
              <li><a href="#servicos" className="hover:text-brand-medium transition-colors touch-target inline-block">Terraplanagem</a></li>
            </ul>
          </div>
        ...
        <div className="pt-8 md:pt-12 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-8">
          <p className="text-[10px] md:text-xs uppercase tracking-widest font-bold text-white/30 text-center md:text-left">
            © 2026 Mix Construtora. Todos os direitos reservados.
          </p>
          
          <div className="flex gap-6 md:gap-10">
            <a href="#" className="text-white/30 hover:text-brand-medium transition-colors touch-target flex items-center justify-center"><FileText size={18} md:size={20} /></a>
            <a href="#" className="text-white/30 hover:text-brand-medium transition-colors touch-target flex items-center justify-center"><ShieldCheck size={18} md:size={20} /></a>
          </div>
        </div>
      </div>
    </footer>
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <button
            onClick={scrollToTop}
            className="w-12 h-12 md:w-16 md:h-16 bg-white text-teal-deep rounded-full flex items-center justify-center shadow-2xl hover:bg-brand-medium hover:text-white transition-all duration-300 border border-teal-deep/5 touch-target"
